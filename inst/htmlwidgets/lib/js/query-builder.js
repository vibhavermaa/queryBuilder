...
var Selectors = QueryBuilder.selectors = {
    ...
    //added by vibha
    filter_type_container:'.rule-type-container',
    ...
    //added by vibha
    rule_filter_type:     '.rule-type-container [name$=_filterType]',
...
QueryBuilder.DEFAULTS = {
    ...
    select_placeholder: '------',
    //added by Vibha
    filter_type_placeholder: 'Please select type',
...
QueryBuilder.prototype.init = function($el, options) {
    ...
    this.filterOptGroups = this.settings.filterOptGroups;
...
QueryBuilder.prototype.bindEvents = function() {
    ...
    //added by vibha
    this.$el.on('change.queryBuilder', Selectors.rule_filter_type, function() {
        var $rule = $(this).closest(Selectors.rule_container);
        $rule.find(Selectors.filter_container).show();
        Model($rule).filter = null;
        const sortedFilters = $(this).val() !== '-1' ? self.settings.filters.filter((obj) => obj.optgroup === $(this).val()) : self.settings.filters;
        self.setFilters(true, sortedFilters, Model($rule));
        self.updateRuleFilter(Model($rule));
    });
...
QueryBuilder.prototype.addRule = function(parent, data, flags) {
    ...
    // Render filter type container - added by vibha
    this.createRuleFilterType(model);
...
QueryBuilder.prototype.createRuleFilterType = function(rule) {
    var $filterTypeSelect = $(this.getRuleFilterTypeSelect(rule, this.filterOptGroups));
    rule.$el.find(Selectors.filter_type_container).html($filterTypeSelect);
    this.trigger('afterCreateRuleFilterType', rule);
};
...
QueryBuilder.prototype.createRuleInput = function(rule) {
    ...
    for (var i = 0; i < rule.operator.nb_inputs; i++) {  //append value container based on nb_inputs
    ...
    // added by vibha - for each input having index and ip
    if(Array.isArray(filter.inputs)) {
        $inputs.each((index, ip) => {
            var currInput = filter.inputs[index];
            if(currInput.plugin) {
                $(ip)[currInput.plugin](currInput.plugin_config || {});
            }
        });
    }
    //end
...
QueryBuilder.prototype.validateValueInternal = function(rule, value) {
    ...
    for (var i = 0; i < operator.nb_inputs; i++) {
        ...
        if (Array.isArray(filter.inputs)) {
            _filter = filter.inputs[i] || {};
            _validation = _filter.validation || {};
        }
        switch (_filter.input) {
            case 'radio':
            ...
            case 'select':
                if (_filter.multiple) {
                    if ( value[i] === undefined || value[i].length === 0 || (_filter.placeholder && value[i] == _filter.placeholder_value)) {
                        result = ['select_empty'];
                        break;
                    }
                    else if (!operator.multiple && value[i].length > 1) {
                    ...
                    if (value[i] === "undefined" || value[i] === undefined || (_filter.placeholder && value[i] == _filter.placeholder_value)) {
                        result = ['select_empty'];
                        break;
                    }
                }
            ...
            case 'selectize':
                ...
                break;
            default:
                switch (QueryBuilder.types[_filter.type]) {
                    case 'string':
                        if (value[i] === undefined || value[i].length === 0) {
                            result = ['string_empty'];
                            break;
                        }
                        if (_validation.min !== undefined) {
                            if (value[i].length < parseInt(_validation.min)) {
                                result = ['string_exceed_min_length', _validation.min];
                                break;
                            }
                        }
                        if (_validation.max !== undefined) {
                            if (value[i].length > parseInt(_validation.max)) {
                                result = ['string_exceed_max_length', _validation.max];
                                break;
                            }
                        }
                        if (_validation.format) {
                            if (typeof _validation.format == 'string') {
                                _validation.format = new RegExp(_validation.format);
                            }
                            if (!_validation.format.test(value[i])) {
                                result = ['string_invalid_format', _validation.format];
                                break;
                            }
                        }
                        break;
                    ...
                    case 'number':
                        if (value[i] === undefined || isNaN(value[i])) {
                            result = ['number_nan'];
                            break;
                        } //change filter to _filter by Vibha
                        if (_filter.type == 'integer') {
                        ...
                        if (_validation.min !== undefined) {
                            if (value[i] < parseFloat(_validation.min)) {
                                result = ['number_exceed_min', _validation.min];
                                break;
                            }
                        }
                        if (_validation.max !== undefined) {
                            if (value[i] > parseFloat(_validation.max)) {
                                result = ['number_exceed_max', _validation.max];
                                break;
                            }
                        }
                        if (_validation.step !== undefined && _validation.step !== 'any') {
                            var v = (value[i] / _validation.step).toPrecision(14);
                            if (parseInt(v) != v) {
                                result = ['number_wrong_step', _validation.step];
                                break;
                            }
                        }
                        break;
                    ...
                    case 'datetime':
                        ...
                        if (_validation.format) {
                            ...
                            var datetime = moment(value[i], _validation.format);
                            if (!datetime.isValid()) {
                                result = ['datetime_invalid', _validation.format];
                                break;
                            }
                            else {
                                if (_validation.min) {
                                    if (datetime < moment(validation.min, _validation.format)) {
                                        result = ['datetime_exceed_min', _validation.min];
                                        break;
                                    }
                                }
                                if (_validation.max) {
                                    if (datetime > moment(validation.max, _validation.format)) {
                                        result = ['datetime_exceed_max', _validation.max];
                                        break;
                                    }
                                }
                            }
    ...
    if (filter.valueGetter) {
        ...
        for (var i = 0; i < operator.nb_inputs; i++) {
            ...
            switch (_filter.input) {
                ...
                case 'selectize':
                    const selectize = $value.find('[name=' + name + ']')[0].selectize;
                    value.push(selectize.items);
                    break;
    ...
    if (filter.valueSetter) {
        ...
        for (var i = 0; i < operator.nb_inputs; i++) {
            ...
            switch (_filter.input) {
                ...
                case 'selectize':
                    const selectize = $value.find('[name=' + name + ']')[0].selectize;
                    selectize.setValue(value[i]);
                    break;

                case 'select':
                    const $select = $value.find('[name=' + name + ']');
                    const options = Array.from($select[0].options).map(op => op.value);
                    if (options.indexOf(String(value[i])) !== -1) {
                        $select.val(String(value[i])).trigger('change');
                    }
                    break;
...
//added by vibha
QueryBuilder.templates.filterTypeSelect = '\
{{ var optgroup = null; }} \
<select class="form-control" name="{{= it.rule.id }}_filterType"> \
  {{? it.settings.display_empty_filter }} \
    <option value="-1">{{= it.settings.select_placeholder }}</option> \
  {{?}} \
  {{~ it.filterOptGroups: filterOptGp }} \
    <option value="{{= filterOptGp }}">{{= it.translate(filterOptGp) }}</option> \
  {{~}} \
</select>';
...
QueryBuilder.prototype.getRuleFilterTypeSelect = function(rule, filterOptGroups) {
    var h = this.templates.filterTypeSelect({
        builder: this,
        rule: rule,
        filterOptGroups: filterOptGroups,
        icons: this.icons,
        lang: this.lang,
        settings: this.settings,
        translate: this.translateLabel
    ...
    return this.change('getRuleFilterTypeSelect', h, rule);
};
/**
...
QueryBuilder.prototype.getRuleInput = function(rule, value_id) {
    ...
    //by vibha
    if(Array.isArray(filter.inputs)) {
        var currInput = filter.inputs[value_id] || {};
        var _validation = currInput.validation || {};
        h = getHValue(currInput, _validation);
    } else {
        h = getHValue(filter, validation)
    }
    //--getH: start
    function getHValue(filter, validation) {
        var h = '';
        if (typeof filter.input == 'function') {
            h = filter.input.call(this, rule, name);
        } else {
            switch (filter.input) {//end
            case 'radio': case 'checkbox':
                h='<span>';
...
QueryBuilder.define('bt-selectpicker', function(options) {
    ...
    // added by vibha
    this.on('afterCreateRuleFilterType', function(e, rule) {
        rule.$el.find(Selectors.rule_filter_type).removeClass('form-control').selectpicker(options);
    });
...
QueryBuilder.extend({
     ...
     */
    setFilters: function(delete_orphans, filters, $rule) {
        var self = this;
        ...
        // check for orphans - code changed by Vibha
       if (!delete_orphans && !$rule) {
        ...
        (function updateBuilder(node) {
            node.each(true,
              function(rule) { //changed by Vibha
                  if (!$rule && rule.filter && filtersIds.indexOf(rule.filter.id) === -1) {
                      rule.drop();
                  }
                  //changed by Vibha
                 else if (!$rule || ($rule && $rule.id === rule.id)) {
